  ...
  <div class="relative flex min-h-screen flex-col md:flex-row">
    ...
    <aside class="glass w-full shrink-0 border-x-0 border-t-0 md:w-64 md:border-y-0 md:border-e">
      ...
      <nav :class="['flex-col gap-1 px-2 pb-4', sidebarOpen ? 'flex' : 'hidden md:flex']">
        <NuxtLink to="/instructor-panel/courses" class="rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-white/40 dark:text-gray-200 dark:hover:bg-white/10">
          دوره‌های من
        </NuxtLink>
        <NuxtLink to="/instructor-panel/courses/create" class="rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-white/40 dark:text-gray-200 dark:hover:bg-white/10">
          دوره‌ی جدید
        </NuxtLink>
        <button
